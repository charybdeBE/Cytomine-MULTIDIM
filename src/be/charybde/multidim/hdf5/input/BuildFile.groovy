...
public class BuildFile {
    ...
    public BuildFile(String filename, int tile_width, int tile_height, int tile_depth, String root, String[] ex) {
    ...
    public BuildFile(String filename, String root, String[] ex) {
        this(filename, 256,256,256, root, ex);
    }


    public void createFile(){
        int nr_width = ed.getImageWidth() / tile_width;
        int nr_height = ed.getImageHeight() / tile_height;
        int nr_depth = ed.getImageDepth() / tile_depth;

        String meta_group = "/meta";
        int[] meta_info = [tile_width, tile_depth, tile_height];
        writer.int32().writeArray(meta_group, meta_info, ft);

        System.out.println(nr_width +"," + nr_height);
        for(int d = 0; d < nr_depth; d++){
            for(int x = 0; x < nr_width; x++){
                for(int y = 0; y < nr_height; y++){
                    ...
                    System.out.println("Tile done : " + group_name + " in (ms) " + stop);
